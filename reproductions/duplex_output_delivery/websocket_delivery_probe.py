...
async def observe(omni, args):
    ...
    summary = {
        ...
        "note": "Client pauses before cancel and 2s after it; observation time is not engine cancel latency. ASGI send acceptance is not physical playback.",
        "deliberate_pause_before_cancel_s": args.pause_before_s,
        "deliberate_pause_after_cancel_s": 2,
        "initial_text_repetitions": {"slow": 12, "normal": 12},
        "errors": [],
        "unexpected_closes": [],
    }
    held_lock = None
    ...
    def compact(payload, role, phase):
        ...
        return {
            ...
            "audio_bytes": len(base64.b64decode(delta)) if audio and delta else 0,
    ...
    class ObservedHandler(OmniDuplexSessionHandler):
        async def _open(self, websocket, envelope, session_payload, send_json):
            attachment = await super()._open(websocket, envelope, session_payload, send_json)
        ...
        async def _send_event(self, session_id, event, **kwargs):
            ...
            if role == "slow" and isinstance(event, AudioDelta):
                summary["slow_server_audio_attempts"] = summary.get("slow_server_audio_attempts", 0) + 1
                if args.mode == "held" and summary["slow_server_audio_attempts"] == 2:
                    ...
                    held_lock = self._attachment_registry._sessions[session_id].outbound_lock
            ...
            if event is held_event:
                ...
                summary["held_event_in_journal"] = (
                    any(e.payload.get("event_id") == event.event_id for e in state.journal._entries)
    ...
    async def observed_app(scope, receive, send):
        ...
        async def observed_send(message):
            ...
            if message["type"] == "websocket.send" and message.get("text"):
                server_events.append(compact(json.loads(message["text"]), role, "asgi_send_accepted"))
    ...
    async def receive(role):
        try:
            async for raw in sockets[role]:
                ...
                if entry["type"] in {"session.closed", "session.expired"} and not closing:
            ...
            if not closing:
                summary["unexpected_closes"].append({"role": role, "at_s": now(), "message": str(exc)})
        finally:
            if not closing:
                summary["unexpected_closes"].append({"role": role, "at_s": now(), "message": "receive loop ended"})
        ...
        for sid, role in roles.items():
            ...
            if handle is not None:
                ...
                result[role] = {
                    ...
                    "pending_events": count if count is not None else handle._outbox.qsize(),
    ...
    try:
        async with asyncio.timeout(240), AsyncExitStack() as stack:
            ...
            for role in ("slow", "normal"):
                ...
                config = SessionConfig(
                    ...
                    extra_body={
                        "auto_response": True,
                        "duplex_initial_user_text": story * summary["initial_text_repetitions"][role],
                        "force_listen_count": 0,
                    },
                ).to_session_payload(model=args.model)
                ref = args.ref_audio or Path(args.model) / "assets" / "HT_ref_audio.wav"
                config["ref_audio"] = "data:audio/wav;base64," + base64.b64encode(ref.read_bytes()).decode()
            ...
            if normal_done.is_set():
                raise RuntimeError("Normal response ended before cancellation; shorten --pause-before-s and rerun.")
            summary["before_cancel"] = buffer_snapshot()
            summary["cancel_submitted_at_s"] = now()
            await send("slow", {"type": "response.cancel", "response_id": responses["slow"]})
            await asyncio.sleep(2)
            summary["after_cancel_before_release"] = buffer_snapshot()
        ...
        for label, source in (("received", events), ("asgi_accepted", server_events)):
            old_audio = [e for e in source if e["role"] == "slow" and e["response_id"] == old and e["audio_bytes"]]
            summary[f"old_audio_{label}_after_cancel_before_terminal"] = sum(
                e["audio_bytes"] for e in old_audio if cancel_at <= e["at_s"] < terminal_at
            )
        ...
        for role in first:
            ...
            seq = [e["server_event_seq"] for e in own if isinstance(e["server_event_seq"], int)]
            terminals = {}
            violations = []
            for event in own:
            ...
            summary[role] = {
                ...
                "audio_during_pause": sum(
                    bool(e["audio_bytes"])
                    for e in own
                    if summary.get("pause_window_started_at_s", float("inf")) <= e["at_s"] < cancel_at
                ),
                "audio_after_cancel": sum(bool(e["audio_bytes"]) for e in own if e["at_s"] >= cancel_at),
                ...
                "initial_response_audio_asgi_after_cancel": sum(
                    ...
                    if e["role"] == role and e["response_id"] == responses.get(role) and e["at_s"] >= cancel_at
                ),
                ...
                "initial_response_audio_asgi_after_cancelled_terminal": sum(
                    bool(e["audio_bytes"])
                    for e in server_events
                    if e["role"] == role and e["response_id"] == responses.get(role) and e["at_s"] >= terminal_at
                ),
                "initial_response_completed": any(
                    e["type"] == "response.done" and e["response_status"] == "completed" for e in initial
                ),
                "other_response_audio_after_cancel": sum(
                    bool(e["audio_bytes"])
                    for e in own
                    if e["response_id"] != responses.get(role) and e["at_s"] >= cancel_at
                ),
                "sequence_strictly_increasing": all(a < b for a, b in zip(seq, seq[1:])),
                "sequence_contiguous": all(b == a + 1 for a, b in zip(seq, seq[1:])),
                "normal_completions": sum(
                    e["response_status"] == "completed" for e in own if e["type"] == "response.done"
                ),
                "audio_after_own_terminal": violations,
            }
        summary["checks"] = {
            "both_sessions_produced_audio": all(first[role].is_set() and responses.get(role) for role in first),
            "slow_cancellation_accepted": cancelled.is_set(),
            "normal_initial_response_audio_during_pause": summary["normal"]["initial_response_audio_during_pause"] > 0,
            "normal_initial_response_audio_after_cancel": summary["normal"]["initial_response_audio_after_cancel"] > 0,
            ...
            "normal_initial_response_audio_asgi_after_cancelled_terminal": (
                summary["normal"]["initial_response_audio_asgi_after_cancelled_terminal"] > 0
            ),
            "normal_initial_response_completed": summary["normal"]["initial_response_completed"],
            "sequences_valid": all(
                summary[role]["sequence_strictly_increasing"] and summary[role]["sequence_contiguous"] for role in first
            ),
            "no_audio_after_own_terminal": all(not summary[role]["audio_after_own_terminal"] for role in first),
            "no_errors_or_unexpected_closes": not summary["errors"] and not summary["unexpected_closes"],
        }
...
def main():
    ...
    parser.add_argument("--ref-audio", type=Path, help="Reference WAV; defaults to the local model's assets/HT_ref_audio.wav")
