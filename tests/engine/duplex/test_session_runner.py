...
@pytest.mark.asyncio
@pytest.mark.parametrize("new_response_started", [False, True])
async def test_clearing_fully_played_response_does_not_interrupt_new_input(new_response_started: bool) -> None:
    h = await open_harness()
    try:
        await h.run(append_audio())
        request_id = h.stage0_request_id()
        await h.deliver_and_settle(tts_output(request_id, text="hello"))
        events = await h.deliver_and_settle(tts_output(request_id, text="hello", turn_end=True))
        previous_response_id = find(events, "response.done").response_id
        await h.run(commands.AckPlayback(played_ms=1000, response_id=previous_response_id))

        await h.run(append_audio())
        if new_response_started:
            await h.deliver_and_settle(tts_output(request_id, text="next", turn_id=1))
        active_response_id = h.session.active_response_id

        events = await h.run(commands.ClearOutputAudio(response_id=previous_response_id))
        assert types(events) == ["output_audio_buffer.cleared"]
        assert events[0].response_id == previous_response_id
        assert h.session.epoch == 0
        assert h.session.active_response_id == active_response_id
